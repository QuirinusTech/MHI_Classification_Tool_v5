...
for chem in bigDatabase:
  MainDB.append({"chemid": chem[0], "CAS": chem[1], "name": chem[2], "tier1": chem[3], "tier2": chem[4], "tier3": chem[5], "chemtype": chem[6], "class": chem[7], "tooltip": chem[8], "hphrases": chem[9]})

for chem in MainDB:
  chemlist.append({"chemid": chem["chemid"], "CAS": chem["CAS"], "name": chem["name"], "chemtype": chem["chemtype"], "class": chem["class"], "tooltip": chem["tooltip"], "hphrases": chem["hphrases"]})
...
#set up final variable
  ...
  #calculate the cumulative rules
  usedlistedsubs = {"A": 0, "B": 0, "C": 0}
  for item in inv:
    if item['chemtype']=="named":
      for substance in MainDB:
        if float(item["chemid"]) == float(substance["chemid"]):
          qx = float(item["qty"]) / float(substance["tier3"])
          rule = hphrases.RuleFinder(item)
          usedlistedsubs[rule] = float(usedlistedsubs[rule]) + qx
