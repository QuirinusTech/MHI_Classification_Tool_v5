namedSubstances = [{
    'name': 'Ammonia anhydrous',
    'UN': '1005',
    'tier1': 15,
    'tier2': 50,
    'tier3': 200,
    'CAS': '7664-41-7',
    'chemid': 222,
    'hazardPhrases': ['H221', 'H314', 'H331', 'H400', 'H280', 'H410', 'H411', 'H318', 'H220', 'H332', 'H334', 'H370', 'H372', 'H373', 'H341']
},
    {
    'name': 'Ammonium nitrate (as described in Note 3)',
    'UN': ['1438', '2067', '2071'],
    'tier1': 2000,
    'tier2': 5000,
    'tier3': 10000,
    'CAS': '6484-52-2',
    'chemid': 22985,
    'hazardPhrases': ['H272', 'H319', 'H372', 'H370', 'H320', 'H335', 'H371', 'H303', 'H315']
},
    {
    'name': 'Ammonium nitrate (as described in Note 4 ) ',
    'UN': '',
    'tier1': 500,
    'tier2': 1250,
    'tier3': 5000,
    'CAS': '6484-52-2',
    'chemid': 22985,
    'hazardPhrases': ['H272', 'H319', 'H372', 'H370', 'H320', 'H335', 'H371', 'H303', 'H315']
},
    {
    'name': 'Ammonium nitrate (as described in Note 5 )',
    'UN': '',
    'tier1': 150,
    'tier2': 350,
    'tier3': 2500,
    'CAS': '6484-52-2',
    'chemid': 22985,
    'hazardPhrases': ['H272', 'H319', 'H372', 'H370', 'H320', 'H335', 'H371', 'H303', 'H315']
},
    {
    'name': 'Ammonium nitrate (as described in Note 6)',
    'UN': '',
    'tier1': 4,
    'tier2': 10,
    'tier3': 50,
    'CAS': '6484-52-2',
    'chemid': 22985,
    'hazardPhrases': ['H272', 'H319', 'H372', 'H370', 'H320', 'H335', 'H371', 'H303', 'H315']
},
    {
    'name': 'Potassium nitrate (as described in Note 7) ',
    'UN': '1486',
    'tier1': 2000,
    'tier2': 5000,
    'tier3': 10000,
    'CAS': '7757-79-1',
    'chemid': 24434,
    'hazardPhrases': ['H272', 'H315', 'H319', 'H335', 'H361', 'H370', 'H372', 'H303', 'H371', 'H373']
},
    {
    'name': 'Potassium nitrate (as described in Note 8) ',
    'UN': '1488',
    'tier1': 500,
    'tier2': 1250,
    'tier3': 5000,
    'CAS': '7757-79-1',
    'chemid': 24434,
    'hazardPhrases': ['H272', 'H315', 'H319', 'H335', 'H361', 'H370', 'H372', 'H303', 'H371', 'H373']
},
    {
    'name': 'Arsenic pentoxide, arsenic (V) acid and/or salts ',
    'UN': '1559',
    'tier1': 1,
    'tier2': 1,
    'tier3': 2,
    'CAS': '1303-28-2',
    'chemid': 14771,
    'hazardPhrases': ['H301', 'H331', 'H350', 'H400', 'H410', 'H300', 'H312', 'H314', 'H341', 'H373', 'H319', 'H361', 'H370', 'H372']
},
    {
    'name': 'Arsenic trioxide, arsenious (III) acid and/or salts ',
    'UN': '1561',
    'tier1': 0.1,
    'tier2': 0.1,
    'tier3': 0.1,
    'hazardPhrases': [],
    'chemid': 14888,
    'CAS': '1327-53-3'
},
    {
    'name': 'Bromine (l)',
    'UN': '1701',
    'tier1': 5,
    'tier2': 20,
    'tier3': 100,
    'CAS': '7726-95-6',
    'chemid': 24408,
    'hazardPhrases': ['H314', 'H330', 'H400', 'H318', 'H290', 'H370', 'H372', 'H301']
}, {
    'name': 'Bromine (a)',
    'UN': '1744',
    'tier1': 5,
    'tier2': 20,
    'tier3': 100,
    'CAS': '7726-95-6',
    'chemid': 24408,
    'hazardPhrases': ['H314', 'H330', 'H400', 'H318', 'H290', 'H370', 'H372', 'H301']
}, {
    'name': 'Chlorine',
    'UN': '1017',
    'tier1': 5,
    'tier2': 10,
    'tier3': 25,
    'hazardPhrases': ['H270', 'H315', 'H319', 'H331', 'H335', 'H400', 'H280', 'H330', 'H410', 'H314', 'H318', 'H370', 'H372', 'H373'],
    'chemid': 24526,
    'CAS': '7782-50-5'
}, {
    'CAS': '1313-99-1',
    'name': 'Nickel monoxide, Nickel dioxide',
    'chemid': 14805,
    'hazardPhrases': ['H317', 'H334', 'H350', 'H360', 'H372', 'H401', 'H411', 'H413'],
    'UN': '3089',
    'tier1': 1,
    'tier2': 1,
    'tier3': 1,
    "tooltip": "Nickel compounds in inhalable powder form (nickel monoxide, nickel dioxide, nickel sulphide, tri-nickel disulphide, di-nickel trioxide)"
}, {
    'name': 'Nickel compounds',
    'tier1': 1,
    'tier2': 1,
    'tier3': 1,
    "tooltip": "Nickel compounds in inhalable powder form (nickel monoxide, nickel dioxide, nickel sulphide, tri-nickel disulphide, di-nickel trioxide)",
    'hazardPhrases': ['H317', 'H334', 'H350', 'H360', 'H372', 'H401', 'H411', 'H413'],
    "chemid": 14805
}, {
    'CAS': '16812-54-7',
    'name': 'Nickel sulphide',
    'chemid': 28094,
    'tier1': 1,
    'tier2': 1,
    'tier3': 1,
    'hazardPhrases': ['H317', 'H332', 'H334', 'H341', 'H350', 'H372', 'H400', 'H410'],
    "tooltip": "Nickel compounds in inhalable powder form (nickel monoxide, nickel dioxide, nickel sulphide, tri-nickel disulphide, di-nickel trioxide)"
}, {
    'name': 'Ethyleneimine',
    'UN': '1185',
    'tier1': 5,
    'tier2': 10,
    'tier3': 20,
    'CAS': '151-56-4',
    'chemid': 9033,
    'hazardPhrases': ['H225', 'H300', 'H310', 'H314', 'H330', 'H340', 'H350', 'H411', 'H318', 'H302', 'H317', 'H319', 'H315', 'H351', 'H361', 'H370', 'H372', 'H335']
}, {
    'name': 'Fluorine',
    'UN': '1045',
    'tier1': 5,
    'tier2': 10,
    'tier3': 20,
    'CAS': '7782-41-4',
    'chemid': 24524,
    'hazardPhrases': ['H270', 'H314', 'H330', 'H280', 'H310', 'H318', 'H315', 'H319', 'H370', 'H372', 'H361']
}, {
    'name': 'Formaldehyde (concentration greater than or equal to 90%)',
    'UN': '1198',
    'tier1': 2.5,
    'tier2': 5,
    'tier3': 50,
    'CAS': '50-00-0',
    'chemid': 712,
    'hazardPhrases': ['H301', 'H311', 'H314', 'H317', 'H331', 'H341', 'H350', 'H228', 'H302', 'H312', 'H315', 'H318', 'H319', 'H332', 'H334', 'H335', 'H351', 'H330', 'H220', 'H280', 'H360', 'H372', 'H401', 'H227', 'H370', 'H402', 'H412']
}, {
    'name': 'Hydrogen',
    'UN': '1049',
    'tier1': 2.5,
    'tier2': 5,
    'tier3': 50,
    'hazardPhrases': ['H220', 'H280', 'H281'],
    'chemid': 783,
    'CAS': '1333-74-0'
}, {
    'name': 'Hydrogen chloride (liquefied gas)',
    'UN': '1050',
    'tier1': 5,
    'tier2': 25,
    'tier3': 250,
    'hazardPhrases': ['H314', 'H331', 'H280', 'H290', 'H318', 'H335', 'H301', 'H330', 'H334', 'H370', 'H372'],
    'chemid': 313,
    'CAS': '7647-01-0'
}, {
    'name': 'Hydrogen fluoride',
    'UN': '1052',
    'tier1': 2.5,
    'tier2': 5,
    'tier3': 20,
    'CAS': '7664-39-3',
    'chemid': 14917,
    'hazardPhrases': ['H300', 'H310', 'H314', 'H330', 'H280', 'H318', 'H331', 'H370', 'H372', 'H402', 'H317', 'H341']
}, {
    'name': 'Lead alkyls',
    'UN': '-',
    'tier1': 2.5,
    'tier2': 5,
    'tier3': 50,
    "chemid": 9048405386,

}, {
    'chemid': 5950,
    'name': 'Extremely flammable gases and natural gas',
    'UN': '1075',
    'tier1': 20,
    'tier2': 50,
    'tier3': 200,
    "tooltip": "Liquid Petroleum Gas (LPG), butane, propane, pentane, dimethyl ether. Liquefied extremely flammable gases (including LPG) and natural gas (whether liquefied or not).",
    'hazardPhrases': ['H220']
}, {
    'name': 'Acetylene',
    'UN': '1001',
    'tier1': 2.5,
    'tier2': 5,
    'tier3': 50,
    'hazardPhrases': ['H220', 'H230', 'H280', 'H336'],
    'chemid': 6326,
    'CAS': '74-86-2'
}, {
    'name': 'Ethylene oxide',
    'UN': '3089',
    'tier1': 2.5,
    'tier2': 5,
    'tier3': 50,
    'CAS': '75-21-8',
    'chemid': 6354,
    'hazardPhrases': ['H220', 'H315', 'H319', 'H331', 'H335', 'H340', 'H350', 'H317', 'H318', 'H411', 'H412', 'H230', 'H280', 'H301', 'H302', 'H314', 'H336', 'H360', 'H372', 'H370', 'H373']
}, {
    'name': 'Propylene oxide',
    'UN': '1280',
    'tier1': 2.5,
    'tier2': 5,
    'tier3': 50,
    'hazardPhrases': ['H224', 'H302', 'H311', 'H319', 'H331', 'H335', 'H340', 'H350', 'H312', 'H315', 'H332', 'H314', 'H317', 'H318', 'H336', 'H341', 'H351', 'H361', 'H330'],
    'chemid': 6378,
    'CAS': '75-56-9'
}, {
    'name': 'Methanol',
    'UN': '1230',
    'tier1': 50,
    'tier2': 500,
    'tier3': 5000,
    'hazardPhrases': ['H225', 'H301', 'H311', 'H331', 'H370', 'H302', 'H319', 'H336', 'H360', 'H372', 'H303', 'H335'],
    'chemid': 887,
    'CAS': '67-56-1'
}, {
    'name': '4, 4-Methylenebis (2-chloraniline) and/or salts, in powder form',
    'UN': '3077',
    'tier1': 0.01,
    'tier2': 0.01,
    'tier3': 0.01,
    'CAS': "101-61-1",
    'chemid': 7567,
    'hazardPhrases': ['H350', 'H400', 'H410'],
}, {
    'name': 'Methyl isocyanate',
    'UN': '2480',
    'tier1': 0.15,
    'tier2': 0.15,
    'tier3': 0.15,
    'hazardPhrases': ['H225', 'H301', 'H311', 'H315', 'H317', 'H318', 'H330', 'H334', 'H335', 'H361', 'H314', 'H341', 'H372', 'H360', 'H370', 'H310'],
    'chemid': 12228,
    'CAS': '624-83-9'
}, {
    'name': 'Oxygen',
    'UN': ['1072 (compressed)', '1073 (refrigerated)'],
    'tier1': 50,
    'tier2': 200,
    'tier3': 2000,
    'hazardPhrases': ['H270', 'H280', 'H281', 'H335', 'H361'],
    'chemid': 977,
    'CAS': '7782-44-7'
}, {
    'name': 'Toluene di-isocyanate',
    'UN': '2078',
    'tier1': 1,
    'tier2': 10,
    'tier3': 100,
    'hazardPhrases': ['H315', 'H317', 'H318', 'H319', 'H330', 'H334', 'H335', 'H351', 'H412', 'H332', 'H371', 'H372'],
    'chemid': 11443,
    'CAS': '584-84-9'
}, {
    'name': 'Carbonyl dichloride (phosgene)',
    'UN': '1076',
    'tier1': 0.3,
    'tier2': 0.3,
    'tier3': 0.75,
    'hazardPhrases': ['H314', 'H330', 'H280', 'H318', 'H315', 'H319', 'H370', 'H372'],
    'chemid': 6371,
    'CAS': '75-44-5'
}, {
    'name': 'Arsenic trihydride (arsine)',
    'UN': '2188',
    'tier1': 0.2,
    'tier2': 0.2,
    'tier3': 1,
    'hazardPhrases': ['H301', 'H331', 'H400', 'H410', 'H315', 'H318', 'H350', 'H341', 'H373', 'H302', 'H319', 'H361', 'H370', 'H372', 'H360'],
    'chemid': 5359596,
    'CAS': '7440-38-2'
}, {
    'name': 'Phosphorus trihydride (phosphine)',
    'UN': '2199',
    'tier1': 0.2,
    'tier2': 0.2,
    'tier3': 1,
    'hazardPhrases': ['H220', 'H314', 'H330', 'H400', 'H280', 'H318', 'H370'],
    'chemid': 24404,
    'CAS': '7803-51-2'
}, {
    'name': 'Sulphur dichloride',
    'UN': '1828',
    'tier1': 1,
    'tier2': 1,
    'tier3': 1,
    'CAS': '10545-99-0',
    'chemid': 25353,
    'hazardPhrases': ['H314', 'H335', 'H400', 'H318', 'H371', 'H373']
}, {
    'name': 'Sulphur dioxide',
    'UN': '1079',
    'tier1': 2.5,
    'tier2': 5,
    'tier3': 20,
    'hazardPhrases': ['H314', 'H331', 'H280', 'H318', 'H319', 'H370', 'H372'],
    'chemid': 1119,
    'CAS': '7446-09-5'
}, {
    'name': 'Sulphur trioxide',
    'UN': '1829',
    'tier1': 7.5,
    'tier2': 15,
    'tier3': 75,
    'CAS': '7446-11-9',
    'chemid': 24682,
    'hazardPhrases': ['H314', 'H318', 'H330', 'H335', 'H351', 'H411']
}, {
    'name': 'Polychlorodibenzofurans and polychlorodibenzodioxins (including TCDD), calculated in TCDD equivalent (See Note 8)',
    'UN': '',
    'tier1': 0.001,
    'tier2': 0.001,
    'tier3': 0.001,
    'tooltip': "2,3,7,8-Tetrachlorodibenzo-p-dioxin (TCDD) 1746-01-6, 1,2,3,7,8-Pentachlorodibenzo-p-dioxin (PeCDD) 40321-76-4, 1,2,3,4,7,8-Hexachlorodibenzo-p-dioxin (HxCDD) 39227-28-6, 1,2,3,6,7,8-Hexachlorodibenzo-p-dioxin (HxCDD) 57653-85-7, 1,2,3,7,8,9-Hexachlorodibenzo-p-dioxin (HxCDD) 19408-74-3, 1,2,3,4,6,7,8-Heptachlorodibenzo-p-dioxin (HpCDD) 35822-46-9, 1,2,3,4,5,6,7,8-Octachlorodibenzo-p-dioxin (OCDD) 3268-87-9, 2,3,7,8-Tetrachlorodibenzofuran (TCDF) 51207-31-9, 1,2,3,7,8-Pentachlorodibenzofuran (PeCDF) 57117-41-6, 2,3,4,7,8-Pentachlorodibenzofuran (PeCDF) 57117-31-4, 1,2,3,4,7,8-Hexachlorodibenzofuran (HxCDF) 70648-26-9, 1,2,3,6,7,8-Hexachlorodibenzofuran (HxCDF) 57117-44-9, 1,2,3,7,8,9-Hexachlorodibenzofuran (HxCDF) 72918-21-9, 2,3,4,6,7,8-Hexachlorodibenzofuran (HxCDF) 60851-34-5, 1,2,3,4,6,7,8-Heptachlorodibenzofuran (HpCDF) 67562-39-4, 1,2,3,4,7,8,9-Heptachlorodibenzofuran (HpCDF) 55673-89-7, 1,2,3,4,5,6,7,8-Octachlorodibenzofuran (OCDF) 39001-02-08290A, Total Tetrachlorodibenzo-p-dioxin (TCDD) 41903-57-5, Total Pentachlorodibenzo-p-dioxin (PeCDD) 36088-22-9, Total Hexachlorodibenzo-p-dioxin (HxCDD) 34465-46-8, Total Heptachlorodibenzo-p-dioxin (HpCDD) 37871-00-4, Total Tetrachlorodibenzofuran (TCDF) 55722-27-5, Total Pentachlorodibenzofuran (PeCDF) 30402-15-4, Total Hexachlorodibenzofuran (HxCDF) 55684-94-1, Total Heptachlorodibenzofuran (HpCDF) 38998-75-3",
    'chemid': 390010208290 
}, {
    'name': 'CARCINOGENS',
    'tooltip': '4-Aminobiphenyl and/or its salts, Benzotrichloride, Benzidine and/or salts, Bis (chloromethyl) ether, Chloromethyl methyl ether, 1,2- Dibromoethane, Diethyl sulphate, Dimethyl sulphate, Dimethylcarbamoyl chloride, 1,2-Dibromo-3- chloropropane, 1,2- Dimethylhydrazine, Dimethylnitrosamine, Hexamethylphosphoric triamide, Hydrazine, 2- Naphthylamine and/or salts, 4-Nitrodiphenyl and 1,3-Propanesultone',
    'tier1': 0.5,
    'tier2': 0.5,
    'tier3': 2,
    "chemid": 59304100063,
}, {
    'name': 'Petroleum  (GAS)',
    'UN': '1075',
    "tooltip": "Diesel, petrol, paraffin, naphtha, kerosene, jet fuel, aviation fuel, light fuel oil. Petroleum products: gasolines , naphthas, kerosenes (including jet fuels), gas oils (including diesel fuels, home heating oils and gas oil blending streams) ",
    'tier1': 250,
    'tier2': 2500,
    'tier3': 25000,
    "chemid": 22120001075
}, {
    'name': 'Petroleum Products (CRUDE)',
    'UN': '1275',
    "tooltip": "Diesel, petrol, paraffin, naphtha, kerosene, jet fuel, aviation fuel, light fuel oil. Petroleum products: gasolines , naphthas, kerosenes (including jet fuels), gas oils (including diesel fuels, home heating oils and gas oil blending streams) ",
    'tier1': 250,
    'tier2': 2500,
    'tier3': 25000,
    "chemid": 22120001275
    
}, {
    'name': 'Boron trifluoride',
    'UN': '1008',
    'tier1': 5,
    'tier2': 5,
    'tier3': 20,
    'hazardPhrases': ['H314', 'H330', 'H280', 'H318', 'H335', 'H373', 'H371', 'H372', 'H336'],
    'chemid': 6356,
    'CAS': '7637-07-2'
}, {
    'name': 'Hydrogen sulphide',
    'UN': '1053',
    'tier1': 5,
    'tier2': 5,
    'tier3': 20,
    'hazardPhrases': ['H220', 'H330', 'H400', 'H280', 'H319', 'H370'],
    'chemid': 402,
    'CAS': '7783-06-4'
}, {
    'name': 'Piperidine',
    'UN': '2401',
    'tier1': 20,
    'tier2': 50,
    'tier3': 200,
    'hazardPhrases': ['H225', 'H311', 'H314', 'H331', 'H302', 'H318', 'H361', 'H370'],
    'chemid': 8082,
    'CAS': '110-89-4'
}, {
    'chemid': 6329,
    'CAS': '74-89-5',
    'name': 'Bis(2-dimethylaminoethyl) (methyl)amine',
    'UN': '-',
    'tier1': 20,
    'tier2': 50,
    'tier3': 200,
    'hazardPhrases': ['H220', 'H224', 'H225', 'H280', 'H302', 'H314', 'H315', 'H318', 'H319', 'H332', 'H335', 'H331', 'H301', 'H370', 'H373', 'H340']
}, {
    'CAS': '5397-31-9',
    'name': '3-(2-Ethylhexyloxy) propylamine',
    'UN': '',
    'tier1': 20,
    'tier2': 50,
    'tier3': 200,
    'hazardPhrases': ['H302', 'H311', 'H314'],
    'chemid': 53973190001
}, {
    'name': 'Propylamine',
    'UN': '1277',
    'tier1': 200,
    'tier2': 500,
    'tier3': 2000,
    'CAS': '107-10-8',
    'chemid': 7852,
    'hazardPhrases': ['H225', 'H290', 'H302', 'H311', 'H331', 'H314', 'H318', 'H335', 'H412', 'H410', 'H312']
}, {
    'name': 'Tert-butyl acrylate',
    'tier1': 100,
    'tier2': 200,
    'tier3': 500,
    'CAS': '1663-39-4',
    'chemid': 15458,
    'hazardPhrases': ['H225', 'H302', 'H312', 'H315', 'H317', 'H332', 'H335', 'H411', 'H331', 'H370', 'H320', 'H373']
}, {
    'name': '2-Methyl-3-butenenitrile',
    'tier1': 200,
    'tier2': 500,
    'tier3': 2000,
    'CAS': '16529-56-9',
    'chemid': 27909,
    'hazardPhrases': ['H225', 'H302', 'H332']
}, {
    'name': 'Tetrahydro-3,5-dimethyl-1,3,5-thiadiazine-2-thione (Dazomet)',
    'UN': '1277',
    'tier1': 50,
    'tier2': 100,
    'tier3': 200,
    'hazardPhrases': ['H225', 'H290', 'H302', 'H311', 'H331', 'H314', 'H318', 'H335', 'H412', 'H410'],
    'chemid': 7852,
    'CAS': '107-10-8'
}, {
    'name': 'Methyl acrylate',
    'UN': '1919',
    'tier1': 200,
    'tier2': 500,
    'tier3': 2000,
    'hazardPhrases': ['H225', 'H302', 'H312', 'H315', 'H317', 'H319', 'H332', 'H335', 'H331', 'H401', 'H412', 'H301', 'H314', 'H318', 'H341', 'H370', 'H372', 'H373'],
    'chemid': 7294,
    'CAS': '96-33-3'
}, {
    'name': '3-Methylpyridine',
    'UN': '2313',
    'tier1': 200,
    'tier2': 500,
    'tier3': 2000,
    'CAS': '108-99-6',
    'chemid': 7970,
    'hazardPhrases': ['H226', 'H302', 'H311', 'H331', 'H314', 'H318', 'H319', 'H332', 'H335', 'H336', 'H351', 'H372', 'H402', 'H411', 'H370']
}, {
    'name': '1-Bromo-3-chloropropane',
    'UN': '2688',
    'tier1': 200,
    'tier2': 500,
    'tier3': 2000,
    'hazardPhrases': ['H226', 'H302', 'H315', 'H319', 'H331', 'H332', 'H335', 'H341', 'H412', 'H351', 'H371', 'H373'],
    'chemid': 8006,
    'CAS': '109-70-6'
}]

succeeded = ['Ammonia anhydrous', 'Ammonium nitrate (as described in Note 3)', 'Ammonium nitrate (as described in Note 4 ) ', 'Ammonium nitrate (as described in Note 5 )', 'Ammonium nitrate (as described in Note 6)', 'Potassium nitrate (as described in Note 7) ', 'Potassium nitrate (as described in Note 8) ', 'Arsenic pentoxide, arsenic (V) acid and/or salts ', 'Bromine (l)', 'Bromine (a)', 'Chlorine', 'Ethyleneimine', 'Fluorine', 'Formaldehyde (concentration greater than or equal to 90%)', 'Hydrogen', 'Hydrogen chloride (liquefied gas)', 'Hydrogen fluoride', 'Extremely flammable gases and natural gas', 'Acetylene', 'Ethylene oxide', 'Propylene oxide', 'Methanol', 'Methyl isocyanate', 'Oxygen', 'Toluene di-isocyanate', 'Carbonyl dichloride (phosgene)', 'Arsenic trihydride (arsine)', 'Phosphorus trihydride (phosphine)', 'Sulphur dioxide', 'Sulphur trioxide', 'Boron trifluoride', 'Hydrogen sulphide', 'Piperidine', 'Bis(2-dimethylaminoethyl) (methyl)amine', 'Propylamine', 'Tert-butyl acrylate', '2-Methyl-3-butenenitrile', 'Tetrahydro-3,5-dimethyl-1,3,5-thiadiazine-2-thione (Dazomet)', 'Methyl acrylate', '3-Methylpyridine', '1-Bromo-3-chloropropane']